class ManagementsController < ApplicationController
  ...
  def index
    if params[:nursing_home_id].nil?
      ...
      params[:offset] ||= 0
      params[:limit] ||= 1000
      @managements = NursingHome.find_by_id(params[:nursing_home_id]).managements.order("id desc").offset(params[:offset]).limit(params[:count])
    end
  end
  ...
  private
    ...
    def management_params
      params.require(:management).permit(:nursing_home_id, :user_id)
    end
end
